...
##Follow the link https://developers.google.com/identity/sign-in/web/devconsole-project to 
##get following variable values. These are needed for configuring ADS server. 
## Default values are for =base10= cluster name
...
sed -i "s|consumer_key:.*|consumer_key: $CONSUMER_KEY|g" "$COMMONVARS_PATH"
sed -i "s|consumer_secret:.*|consumer_secret: $CONSUMER_SECRET|g" "$COMMONVARS_PATH"
