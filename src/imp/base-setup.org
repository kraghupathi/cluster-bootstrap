...
---
   ...
   - copy: Copying ifcfg-br0 template to /etc/sysconfig/network-scripts/ifcfg-br0 on host machine
       content: ""      
...
#   - name: Bridge br0 network interface up
#    command: ifup br0
#    register: out
...
---
   ...
   - copy:  Copying ifcfg-eth0 template to /etc/sysconfig/network-scripts/ifcfg-eth0 on host machine
      content: ""      
...
- name: clonning systems-model repo
  git: repo="{{systems_model_repo}}" dest="{{systems_model_dest}}" version=cluster-automation accept_hostkey=yes
  environment: "{{proxy_env}}"
...
---
    ...
    - name: running install_openvz.sh shell script
