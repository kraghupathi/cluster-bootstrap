...
- name: clonning systems-model repo from gitlab
...
*** SSH configuration to clone to systems-model from bitbucket.org 
    ...
    systems-model from http://gitlab.org
...
**** Clone setup-ovpl-centos repository
...
#    - Clones [[./https://github.com/vlead/cluster-setup-ovpl-centos][cluster-setup-ovpl-centos]] repository from
    - Clones [[./https://github.com/vlead/setup-ovpl-centos][setup-ovpl-centos]] repository from
...
---
    ...
    - name: Comment install dependencies 
      command: 'sed -i "/install_dependencies/s/^/#/g" ~/setup-ovpl-centos/scripts/centos_prepare_ovpl.sh'
      ignore_errors: yes
      register: out
    - debug: var=out.stdout_lines
   
    - name: Comment install mongodb 
      command: 'sed -i "/install_mongodb/s/^/#/g" ~/setup-ovpl-centos/scripts/centos_prepare_ovpl.sh'
      ignore_errors: yes
      register: out
    - debug: var=out.stdout_lines
