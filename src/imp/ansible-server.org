...
- hosts: ads_server
  ...
  tasks:
   - name: Cloning setup-ovpl-centos repo from github 
     shell: "{{export_proxy}} && git clone {{setup_ovpl_centos}} && cd setup-ovpl-centos &&  git checkout master"
     ignore_errors: yes
     register: out
   - debug: var=out.stdout_lines
   ...
   - name: Downloading python tornado-4.3 file 
     shell:  "{{export_proxy}} && wget https://pypi.python.org/packages/source/t/tornado/tornado-4.3.tar.gz"
   - name: Extract torando-4.3tar.gz file
     unarchive:  src=~/tornado-4.3.tar.gz  dest=~/ remote_src=yes
 
   - name: Run setu.py file
     shell: {{export_proxy}} && python setup.py build
     register: out
     args:
      chdir: ~/tornado-4.3
   - debug: var=out.stdout_lines
   ...
   - name: Install certifi dependecy for tornado-4.3
     shell: "{{export_proxy}} && wget https://pypi.python.org/packages/20/d0/3f7a84b0c5b89e94abbd073a5f00c7176089f526edb056686751d5064cbd/certifi-2017.7.27.1.tar.gz#md5=48e8370da8b370a16e223ee9c7b6b063 -O ~/certifi.tar.gz"
   ...
   - name: Install tornado dependencies
     shell: "{{export_proxy}} && yum install python2-backports_abc python-singledispatch python-backports-ssl_match_hostname -y"
