...
- name: Configuring all server nodes
  shell: ansible-playbook -i hosts configure_servers.yaml && iptables -A INPUT 5 -m state --state NEW -s {{hostmachine_ip}}/32 -p tcp -m tcp --dport 22 -j ACCEPT && iptables-save > /etc/sysconfig/iptables
  register: out
  args:
     chdir: systems-model/build/{{clustername}}/
- debug: var=out.stdout_lines
...
- hosts: ads_server
  ...
  tasks:
   ...
   - name: Delete extra line
     command: sed '3d' ovpl/src//ads-web-app/config.py
